...
// And connect to the path in "lib/my_voice_app_web/endpoint.ex". We pass the
...
//
// In your "lib/my_voice_app_web/router.ex":
//
//     pipeline :browser do
//       ...
//       plug MyAuth
...
// inside a script tag in "lib/my_voice_app_web/templates/layout/app.html.heex":
//
//     <script>window.userToken = "<%= assigns[:user_token] %>";</script>
//
// You will need to verify the user token in the "connect/3" function
// in "lib/my_voice_app_web/channels/user_socket.ex":
//
...
// 通話チャネルの作成
let channel = socket.channel("call:1234", {});
channel.join()
    .receive("ok", resp => { console.log("Joined successfully", resp); })
    .receive("error", resp => { console.log("Unable to join", resp); });
...
peerConnection.onicecandidate = event => {
    if (event.candidate) {
        channel.push("candidate", { candidate: event.candidate });
    }
};
